...
def test_complex_trace():
    print(f"\n{Colors.HEADER}--- COMPLEX MATRIX TRACE TEST ---{Colors.END}\n")

    print(f"{Colors.TEST}Testing trace for matrix [[1+1j, 0], [0, 1]] {Colors.END}", end="\n---\n")
    m = Matrix([[1+1j, 0], [0, 1]])
    expected = 2+1j
    result = m.trace()
    print_comparison("Trace", expected, result)

    print(f"{Colors.TEST}Testing trace for matrix [[2j, -5], [4, 3-1j]] {Colors.END}", end="\n---\n")
    m = Matrix([[2j, -5], [4, 3-1j]])
    expected = 2j + (3-1j)
    result = m.trace()
    print_comparison("Trace", expected, result)

    print(f"{Colors.TEST}Testing trace for matrix [[1, 2j, 3], [4, 5, 6], [7j, 8, 9]] {Colors.END}", end="\n---\n")
    m = Matrix([[1, 2j, 3], [4, 5, 6], [7j, 8, 9]])
    expected = 1 + 5 + 9
    result = m.trace()
    print_comparison("Trace", expected, result)

    print(f"{Colors.TEST}Testing trace for matrix [[-1j, 2], [3, 4j]] {Colors.END}", end="\n---\n")
    m = Matrix([[-1j, 2], [3, 4j]])
    expected = -1j + 4j
    result = m.trace()
    print_comparison("Trace", expected, result)
