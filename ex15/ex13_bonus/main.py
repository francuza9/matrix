...
def test_complex_matrix_rank():
	print(f"\n{Colors.HEADER}--- COMPLEX MATRIX RANK TEST ---{Colors.END}\n")

	print(f"{Colors.TEST}Testing rank for matrix [[1j, 0], [0, 1]] {Colors.END}", end="\n---\n")
	m = Matrix([[1j, 0], [0, 1]])
	expected = 2
	result = m.rank()
	print_comparison("Rank", expected, result)

	print(f"{Colors.TEST}Testing rank for matrix [[1+1j, 2], [2+2j, 4]] {Colors.END}", end="\n---\n")
	m = Matrix([[1+1j, 2], [2+2j, 4]])
	expected = 1
	result = m.rank()
	print_comparison("Rank", expected, result)

	print(f"{Colors.TEST}Testing rank for matrix [[1, 2j, 3], [4, 5, 6j], [7, 8j, 9]] {Colors.END}", end="\n---\n")
	m = Matrix([[1, 2j, 3], [4, 5, 6j], [7, 8j, 9]])
	expected = 3
	result = m.rank()
	print_comparison("Rank", expected, result)


def test_complex_matrix_rank_edge_cases():
	print(f"\n{Colors.HEADER}--- COMPLEX MATRIX RANK EDGE CASES ---{Colors.END}\n")

	print(f"{Colors.TEST}Testing rank for zero complex matrix [[0j, 0], [0, 0j]] {Colors.END}", end="\n---\n")
	m = Matrix([[0j, 0], [0, 0j]])
	expected = 0
	result = m.rank()
	print_comparison("Rank", expected, result)

	print(f"{Colors.TEST}Testing rank for tall matrix [[1j, 2], [2j, 4], [3j, 6]] {Colors.END}", end="\n---\n")
	m = Matrix([[1j, 2], [2j, 4], [3j, 6]])
	expected = 1
	result = m.rank()
	print_comparison("Rank", expected, result)

	print(f"{Colors.TEST}Testing rank for rectangular complex matrix [[1, 2j, 3], [4j, 5, 6]] {Colors.END}", end="\n---\n")
	m = Matrix([[1, 2j, 3], [4j, 5, 6]])
	expected = 2
	result = m.rank()
	print_comparison("Rank", expected, result)

	print(f"{Colors.TEST}Testing rank for dependent rows [[1+1j, 2], [2+2j, 4], [3+3j, 6]] {Colors.END}", end="\n---\n")
	m = Matrix([[1+1j, 2], [2+2j, 4], [3+3j, 6]])
	expected = 1
	result = m.rank()
	print_comparison("Rank", expected, result)

	print(f"{Colors.TEST}Expecting ValueError for ragged complex matrix [[1j, 2], [3]] {Colors.END}", end="\n---\n")
	print_exception("Rank", ValueError, lambda: Matrix([[1j, 2], [3]]))


test_matrix_rank()
test_matrix_rank_edge_cases()
test_complex_matrix_rank()
test_complex_matrix_rank_edge_cases()